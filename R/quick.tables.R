...
quick.reg = function(my.model,
  ...
  if (type == "manova" | type == "stats::manova") {
    ...
    if (type == "lm") {
      ...
      ddeviance2 = sum(my.III.summary$Chisq)
      ddf2 = sum(my.III.summary$Df)
      fit2 = 1 - pchisq(ddeviance2, ddf2)
      total.intercepts = my.temp.ord$edf
      my.rownames = c(abbreviate(rownames(my.summary$coefficients), minlength =
                                   abbrev.length))
    } else{
      print("Error")
      return()
    ...
    if (type == "lm") {
      ...
      glance_stats=as.data.frame(matrix(ncol={7+v.p.rep},nrow=4))
